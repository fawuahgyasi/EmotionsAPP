...
#import "LettersViewController2.h"

@interface LettersViewController2 ()
@property(nonatomic) CGPoint selectedImageCenter;
@property(nonatomic) int counter;
@end

@implementation LettersViewController2
...
int const xOffsetLetters2 = 50;
int const yOffsetLetters2 = 30;
int const numberOfOptionsLetters2 = 5;
...
- (BOOL)isInCorrectPosition:(CGPoint)dropPoint:(CGPoint)matchCenter {
    if(dropPoint.x >= matchCenter.x-xOffsetLetters2 && dropPoint.x <= matchCenter.x+xOffsetLetters2 && 
       dropPoint.y >= matchCenter.y-yOffsetLetters2 && dropPoint.y <= matchCenter.y+yOffsetLetters2) {
        return YES;
    } else
        return NO;
}
...
- (void)handlePanEnded:(UIPanGestureRecognizer *)recognizerl {
    ...
    if(self.counter == numberOfOptionsLetters2) {
...
- (void) performSegue:(NSTimer *)timer {
    // Perform segue
    [self performSegueWithIdentifier: @"lettersSegue2" sender: self];
}

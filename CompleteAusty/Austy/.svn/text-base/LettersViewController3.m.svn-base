...
#import "LettersViewController3.h"

@interface LettersViewController3 ()
@property(nonatomic) CGPoint selectedImageCenter;
@property(nonatomic) int counter;
@end

@implementation LettersViewController3
...
int const xOffsetLetters3 = 50;
int const yOffsetLetters3 = 30;
int const numberOfOptionsLetters3 = 5;
...
- (BOOL)isInCorrectPosition:(CGPoint)dropPoint:(CGPoint)matchCenter {
    if(dropPoint.x >= matchCenter.x-xOffsetLetters3 && dropPoint.x <= matchCenter.x+xOffsetLetters3 && 
       dropPoint.y >= matchCenter.y-yOffsetLetters3 && dropPoint.y <= matchCenter.y+yOffsetLetters3) {
        return YES;
    } else
        return NO;
}
...
- (void)handlePanEnded:(UIPanGestureRecognizer *)recognizerl {
    ...
    if(self.counter == numberOfOptionsLetters3) {
...
- (void) performSegue:(NSTimer *)timer {
    // Perform segue
    [self performSegueWithIdentifier: @"lettersSegue3" sender: self];
}

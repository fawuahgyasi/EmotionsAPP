//
//  VehiclesAndColorsViewController.m
//  Austy
//
//  Created by Jose Picado on 3/21/12.
//  Copyright (c) 2012 __MyCompanyName__. All rights reserved.
//

#import "VehiclesAndColorsViewController1.h"

@interface VehiclesAndColorsViewController1 ()
@property(nonatomic) CGPoint selectedImageCenter;
@property(nonatomic) int counter;
@end

@implementation VehiclesAndColorsViewController1
@synthesize match1 = _match1;
@synthesize match2 = _match2;
@synthesize match3 = _match3;
...
- (BOOL)isInCorrectPosition:(CGPoint)dropPoint:(CGPoint)matchCenter {
    if(dropPoint.x >= matchCenter.x-xOffset && dropPoint.x <= matchCenter.x+xOffset && 
       dropPoint.y >= matchCenter.y-yOffset && dropPoint.y <= matchCenter.y+yOffset) {
        return YES;
    } else
        return NO;
}
...
- (void)handlePanEnded:(UIPanGestureRecognizer *)recognizer {
    ...
    if(self.counter == numberOfOptions) {
        [self finishScene];
    }
}
...
- (void)finishScene {
    // Play sound
    [self play:(CFStringRef)@"carSound"];

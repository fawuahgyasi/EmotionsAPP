//
//  VehiclesAndColorsViewController4.m
//  Austy
//
...
#import "VehiclesAndColorsViewController4.h"

@interface VehiclesAndColorsViewController4 ()
@property(nonatomic) CGPoint selectedImageCenter;
@property(nonatomic) int counter;
@end

@implementation VehiclesAndColorsViewController4
@synthesize match1;
@synthesize match2;
@synthesize match3;
@synthesize option1;
...
int const xOffset4 = 50;
int const yOffset4 = 30;
int const numberOfOptions4 = 3;

// Determine if dropPoint is in matchCenter boundaries
- (BOOL)isInCorrectPosition:(CGPoint)dropPoint:(CGPoint)matchCenter {
    if(dropPoint.x >= matchCenter.x-xOffset4 && dropPoint.x <= matchCenter.x+xOffset4 && 
       dropPoint.y >= matchCenter.y-yOffset4 && dropPoint.y <= matchCenter.y+yOffset4) {
        return YES;
    } else
        return NO;
}
...
- (void)handlePanEnded:(UIPanGestureRecognizer *)recognizer {
    ...
    if(self.counter == numberOfOptions4) {
        [self finishScene];
    }
}

- (void)finishScene {
    [self play:(CFStringRef)@"trainSound"];
...
- (void)performLastAnimations {
	...
	[path1 addCurveToPoint:CGPointMake(-100, 200)
             controlPoint1:CGPointMake(50, 200)
             controlPoint2:CGPointMake(25, 200)];
	...
	[path2 addCurveToPoint:CGPointMake(100, 200)
             controlPoint1:CGPointMake(600, 200)
	...
	[path3 addCurveToPoint:CGPointMake(1100, 120)
             controlPoint1:CGPointMake(600, 120)
             controlPoint2:CGPointMake(800, 120)];
...
- (void) performSegue:(NSTimer *)timer {
    // Perform segue
    [self performSegueWithIdentifier: @"vehiclesAndColorsSegue5" sender: self];
}
